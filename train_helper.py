...
def train_flownet_one_epoch(model, train_loader, optimizer, scheduler, device, augment, epoch):
    start = time.time()
    epoch_loss = 0.0
...
def train_flownet(fc_loader, train_loader, device, augment, pretrain=True):
    ...
    if pretrain:
        ...
        for epoch in range(pretrain_epochs):
            pretrain_losses.append(train_flownet_one_epoch(model, fc_loader, optimizer, scheduler, device, augment, epoch))
    print("Finetuning Flownet on", len(train_loader.dataset), "Sintel examples...")
    for epoch in range(finetune_epochs):
        finetune_losses.append(train_flownet_one_epoch(model, train_loader, optimizer, scheduler, device, augment, epoch))
...
def train_raft(fc_loader, train_loader, device, augment, pretrain=True):
    ...
    if pretrain:
        ...
        for epoch in range(pretrain_epochs):
            pretrain_losses.append(train_raft_one_epoch(model, fc_loader, optimizer, scheduler, device, augment, epoch, num_train_flow_updates))
    print("Finetuning RAFT on", len(train_loader.dataset), "Sintel examples...")
    for epoch in range(finetune_epochs):
        finetune_losses.append(train_raft_one_epoch(model, train_loader, optimizer, scheduler, device, augment, epoch, num_train_flow_updates))
    return model, pretrain_losses, finetune_losses
...
def save_results(results, model_type, pretrain_size, augment):
    ...
    path = os.path.join("results", model_type + str(pretrain_size) + augment + "_results.txt")
    torch.save(results, path)
    
def save_model(model, model_type, pretrain_size, augment):
    augment = "A" if augment else "NA"
    path = os.path.join("results", model_type + str(pretrain_size) + augment + "_params.pt")
